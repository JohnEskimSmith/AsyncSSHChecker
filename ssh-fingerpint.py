...
async def _get_server_host_key(target, *, tunnel=(), family=(), flags=0,
                               ...
                               kex_algs=(), config=(), options=None) -> dict | None:
...
async def worker_single_fingerprint(target, semaphore: asyncio.Semaphore, queue_out: asyncio.Queue) -> None:
    ...
    async with semaphore:
        try:
            future_connection = _get_server_host_key(target=target, client_version='AsyncSSHChecker')
            if result := await asyncio.wait_for(future_connection, timeout=10):
    ...
    await queue_out.put((value, status))
...
async def work_with_create_tasks_queue(queue_with_input: asyncio.Queue,
    ...
    while True:
        ...
        if item == CONST_STOP_STAGE:
            ...
            task = asyncio.create_task(worker_single_fingerprint(item, semaphore, queue_out))
    ...
    async with aiofiles_open(filename, mode=mode_write) as file_with_results:
        while True:
            ...
            if status:
                ...
                number_of_failed += 1
            record = ujson_dumps(line)
            await method_write_result(file_with_results, record)
...
async def main(settings):
    ...
    create_tasks = work_with_create_tasks_queue(queue_input, queue_results, queue_prints, task_semaphore)  # execution
